...
void heavyflavorCSVSF(){
  ...
  //---------------------
  TString ofName = Form("csv_rwt_hf_v%i.root",verNum);
...
// //   etaBins.push_back("1.6");
  ...
  ///// jet selections
  TString jetselection2a = "(numJets == 2 && jets_by_pt_2_btagCombinedSecVertex > 0.679 "; //Probe jet being the first_jet
  ...
  TString jetselection2b = "(numJets == 2 && jets_by_pt_1_btagCombinedSecVertex > 0.679 "; //Probe jet being the second_jet
  ...
  ///// extra seletions
//   TString exselection = "((dR_leplep > 0.2) && (mass_leplep > 12) && (isCleanEvent == 1) && (oppositeLepCharge == 1))"; //General dilepton selection
  ...
  TString firstjetb = "( abs(jets_by_pt_1_flavour)==5 )"; 
  TString secondjetb = "( abs(jets_by_pt_2_flavour)==5 )";
  ...
  if (verNum !=0 ) wgtStr += "csvWgtlf*"; // applying lfSFs 
...
//   double Xmax = 1.0; //Upper bound for x-axis
  ...
  histTemp3->Sumw2();
  histTemp4->Sumw2();
  TH1 * sourceHisto3 = (TH1*) Tree1a->Draw(varName1+" >> histTemp3","("+lepselection1a+ " && "+exselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4 = (TH1*) Tree1a->Draw(varName2+" >> histTemp4","("+lepselection1a+ " && "+exselection+" && "+jetselection2b+")","");
  ...
  histTemp7->Sumw2();
  histTemp8->Sumw2();
  TH1 * sourceHisto7 = (TH1*) Tree1b->Draw(varName1+" >> histTemp7","("+lepselection1b+ " && "+exselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto8 = (TH1*) Tree1b->Draw(varName2+" >> histTemp8","("+lepselection1b+ " && "+exselection+" && "+jetselection2b+")","");
  ...
  histTemp11->Sumw2();
  histTemp12->Sumw2();
  TH1 * sourceHisto11 = (TH1*) Tree1c->Draw(varName1+" >> histTemp11","("+lepselection1c+ " && "+exselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto12 = (TH1*) Tree1c->Draw(varName2+" >> histTemp12","("+lepselection1c+ " && "+exselection+" && "+jetselection2b+")","");
  ...
  histTemp5a->Sumw2();
  histTemp6a->Sumw2();
  TH1 * sourceHisto3a = (TH1*) Tree2->Draw(varName1+" >> histTemp3a",lumi+wgtStr+"("+lepselection2+ " && "+firstjetb + " && "+exselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto4a = (TH1*) Tree2->Draw(varName2+" >> histTemp4a",lumi+wgtStr+"("+lepselection2+ " && "+secondjetb +" && "+exselection+" && "+jetselection2b+")","");
  TH1 * sourceHisto5a = (TH1*) Tree2->Draw(varName1+" >> histTemp5a",lumi+wgtStr+"("+lepselection2+ " && !"+firstjetb +" && "+exselection+" && "+jetselection2a+")","");
  TH1 * sourceHisto6a = (TH1*) Tree2->Draw(varName2+" >> histTemp6a",lumi+wgtStr+"("+lepselection2+ " && !"+secondjetb +" && "+exselection+" && "+jetselection2b+")","");

  histTemp3a->Add(histTemp4a); // MC b
  histTemp5a->Add(histTemp6a); // MC non-b
  ...
  //Overlay shapes
  hClone->Draw("histE1");
  TString plotName = Form("hfSF_Pt%i_Eta%i_v%i",iPt,iEta,verNum);
